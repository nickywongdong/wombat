...
The hardware components of the system will include: the use of OBDII which will receive information from multiple onboard car sensors, all related audio connection capabilities (which includes FM radio, bluetooth, auxiliary, and USB), the storing of data for navigation, and logging of all sensor data from the car in hard drives. The NVIDIA TX2 has native support for all hardware that is being used. \par
...
\begin{itemize}
	...
	\item This SRS must force the backup camera linger on the screen for 4-8 seconds after shifting out of reverse, in accordance to federal motor vehicle safety standards.
\end{itemize}
