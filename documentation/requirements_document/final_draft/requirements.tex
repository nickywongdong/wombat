...
\begin{itemize}
	...
	\item Users may switch between controlling media, navigation, and data logging settings at any time.
...
\end{itemize}
	...
	\end{itemize}
		...
		\begin{itemize}
	\item Users may wipe the data stored in the black box. This function is only available after entering a password.
	\item Users may change the password used to wipe black box data. This function is only available after authentication using the current password.
\end{itemize}
\end{itemize}
...
\begin{itemize}
	...
	\item The system will format and log data from the following sources:
	\begin{itemize}
	\item an OBDII dongle plugged into an OBDII port,
	\item a front-facing dashcam,
	\item and an AHRS sensor.
	\end{itemize}
	...
	\item The navigation system must be capable of navigating to addresses within the United States at a minimum.
	\item The system will automatically switch to the backup camera feed when the vehicle is reversing. 
	\item The system will switch off the display when the car turns off, and the Jetson TX2 will enter a low power mode.
	\item The system will also offer a minimum of one of any of the following optional functions:
        	\begin{itemize}
        		\item The system will be able to control supplemental turn signals and backup lights wirelessly.
		...
		\item The system will utilize computer vision algorithms in conjunction with the front dashcam to detect lane lines and provide lane departure warning.
	\end{itemize}
\end{itemize}
...
\textbf{December 1st, 2017:} 
\begin{itemize}
\item Removed clause stating that users will be able to download data log to removable storage medium.
\item Added clause to functional requirements section stating that data log will be saved to a removable storage medium by default.
\end{itemize}

\textbf{March 16th, 2018:} 
\begin{itemize}
\item Removed the hard requirement of having a WiFi toggle within the data settings menu; this is because WiFi connectivity is only needed to fulfill a stretch goal that is now no longer being developed. Stretch goal cannot be developed due to connectivity conflicts with the dashcam system, which needs to use the WiFi radio in AP mode.
\item Expanded data settings menu clause to also include the ability to change the black box password. 
\item Amended clause in external interfaces to stipulate that users may switch between controlling navigation/data logging/media systems at any time.
\item Amended data formatting and log to make the requirement less ambiguous and easier to verify. 
\item Amended navigation system address clause to offer U.S. maps at minimum. 
\item Refined supplemental turn signal stretch goal to be more representative of development. 
\item Expanded description of optional stretch goal for lane departure warning.
\end{itemize}
