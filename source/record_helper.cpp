...
int main(int argc, char *argv[]) {
  ...
  if(runType == "r") {
    sysCmd = "gst-launch-1.0 -e -v udpsrc port=" + to_string(cameraPort) + " ! application/x-rtp, clock-rate=90000, encoding-name=H264, payload=96 ! rtph264depay ! h264parse ! mp4mux ! filesink location='" + loggingDirectory + "/%02d.mp4'";
  }
  else if (runType == "w") {
